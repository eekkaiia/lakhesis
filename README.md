...
There are two undocumented features. One allows a number of frames to be exported at the current screen update interval by pressing the [V] key. The number is set to 600 by default. These 600 images could, for example, be animated with video editing software to produce a 10 second 720p video at 60fps. The second is an experimental feature that tracks the number of sandpile collapses during each iteration and produces an audio tone based on that number relative the the largest number of collapses recorded during the current simulation. The tone will be lower and longer for an 'avalanche' of collapses that traverses the entire lattice. The audio feature can be turned on/off with the [L] key.
